...
// This must be run in a seprate go routine
func runSimulator() {
	for {
		select {
		case <-done:
		...
		case data, ok := <-i2cBus:
			...
			if data.payload == singleByte {
				writeByteData(data.address, data.value)
			}
			if data.payload == multiByte {
				writeBlockData(data.address, data.values)
			}
		}
	}
}

func writeByteData(address, value byte) error {
...
func writeBlockData(address byte, values []byte) error {
	//	b := []byte{address}
	//	b := append(b, values...
	switch address {
	case setPwmValues:
